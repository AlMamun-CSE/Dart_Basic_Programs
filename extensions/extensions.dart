extension<T> on List<T> {
  ...
  List<T> pricesList() => this.map((item) => item).toList();

  //Extension Method: Adding $ prefix
  List<String> priceLabels(String symble) =>
      this.map((item) => "$symble ${item}").toList();

  //Extension Operator: Hike up the price by n
  List<num> operator ^(int n) =>
      this.map((item) => num.parse("${item}") * n).toList();

  //ExtensionOperator: Reduce all prices by half
  List<num> operator -() =>
